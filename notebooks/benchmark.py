...
def spearman_frobenius(real: pl.DataFrame, synth: pl.DataFrame) -> float:
    ...
    numeric_cols = [
        ...
        and real[c].dtype not in (pl.String, pl.Categorical, pl.String)
    ]
...
def marginal_ks_pvalues(real: pl.DataFrame, synth: pl.DataFrame) -> dict[str, float]:
    ...
    for col in real.columns:
        if col not in synth.columns:
            continue
        if real[col].dtype in (pl.String, pl.Categorical, pl.String):
            continue
...
def tstr_gini_gap(
    ...
    cat_features = [c for c in feature_cols if real_df[c].dtype in (pl.String, pl.Categorical, pl.String)]
...
def run_benchmark():
    ...
    numeric_cols = [c for c in real_df.columns if real_df[c].dtype not in (pl.String, pl.Categorical, pl.String)]
